...
def switch_to_VS_compiler(python_path, version=12):
    ...
    with open(name, "r") as f:
        content = f.read()
    content = content.replace("'win-amd64' : 'amd64',", "'win-amd64' : 'x86_amd64',")
    content = content.replace("if majorVersion >= 6:", "if majorVersion >= 6 :\n        majorVersion = " + str(version))
...
def switch_to_mingw_compiler(python_path):
    ...
    @param  python_path     python path
    """
    dirname = os.path.join(folders["python"], "Lib", "distutils")
